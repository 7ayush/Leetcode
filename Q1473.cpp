class Solution {
public:
    int util(vector<int>& houses, vector<vector<int>>& cost,int m,int n,int target,int prev)
    {
        if(m==-1)
        {
        ...
        if(houses[m]!=0)
        {
            if(houses[m]!=prev)target--;            
            return util(houses,cost,m-1,n,target,houses[m]);
        }
        else
        {
            for(int i=0;i<n;i++){
                if(prev == i+1)
                    ans=min(ans,cost[m][i]+util(houses,cost,m-1,n,target,prev));
                else ans=min(ans,cost[m][i]+util(houses,cost,m-1,n,target-1,i+1));
            }
        }
        return ans;
    ...
    {
        int ans=util(houses,cost,m-1,n,target,0);
